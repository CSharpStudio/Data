...
namespace Css.Reflection
{
    ...
    public static class MethodCaller
    {
        ...
        const BindingFlags propertyFlags = BindingFlags.Public | BindingFlags.Instance | BindingFlags.FlattenHierarchy;

        const BindingFlags fieldFlags = BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance;

        static readonly Dictionary<MethodCacheKey, DynamicMemberHandle> _memberCache = new Dictionary<MethodCacheKey, DynamicMemberHandle>();
        ...
        const BindingFlags allLevelFlags = BindingFlags.FlattenHierarchy | BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;

        const BindingFlags oneLevelFlags = BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;

        const BindingFlags ctorFlags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;

        static Dictionary<MethodCacheKey, DynamicMethodHandle> _methodCache = new Dictionary<MethodCacheKey, DynamicMethodHandle>();
        ...
        /// Parameters to pass to method.
        /// </param>
        static object CallMethod(object obj, DynamicMethodHandle methodHandle, params object[] parameters)
        {
        ...
        static bool CallMethodCore(object obj, string method, object[] parameters, bool errorIfNotFound, out object result)
        {
            result = null;
            ...
            else
            {
                ...
                {
                    foreach (var type in objType.GetHierarchy())
                    {
                        var m = methods.Where(p => p.DeclaringType == type);
                        if (m.Count() == 1)
                        {
                            var mh = GetCachedMethod(m.First());
                            if (mh != null && mh.DynamicMethod != null)
                            {
                                found = true;
                                result = CallMethod(obj, mh, parameters);
        ...
        static DynamicMethodHandle GetCachedMethod(MethodInfo info)
        {
        ...
        static DynamicMethodHandle GetCachedMethod(Type objType, string method, params Type[] parameters)
        {
        ...
        /// <returns></returns>
        static List<MethodInfo> FindMethodsOnNullParam(Type objType, string methodName, Type[] paramters)
        {
        ...
        /// <returns></returns>
        static bool IsParamTypeMatched(Type parameterType, Type targetType)
        {
            if (parameterType == null)
            {
                return targetType.IsClass || targetType.IsNullable();
            }

            return targetType.IsAssignableFrom(parameterType) ||
                !parameterType.IsClass && targetType.IgnoreNullable().IsAssignableFrom(parameterType);
        }
        ...
        /// <returns></returns>
        /// <exception cref="System.InvalidProgramException"></exception>
        static MethodInfo GetMethod(Type objectType, string method, params Type[] parameters)
        {
            var result = FindMethod(objectType, method, parameters);
            return result;
        }

        static MethodInfo FindMethodUsingFuzzyMatching(Type objectType, string method, Type[] parameters)
        {
        ...
        /// </param>
        static MethodInfo FindMethod(Type objectType, string method, int parameterCount)
        {
        ...
        /// <param name="parameters">The parameters.</param>
        /// <returns></returns>
        static bool IsParamArray(ParameterInfo[] parameters)
        {
            var l = parameters.Length;

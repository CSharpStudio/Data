...
using Css.Wpf.UI.Controls.AvalonDock.Layout.Serialization;
using System.Reflection;

namespace Css.Wpf.UI.Controls.AvalonDock.Layout
{
    ...
    public abstract class LayoutContent : LayoutElement, IXmlSerializable, ILayoutElementForFloatingWindow, IComparable<LayoutContent>, ILayoutPreviousContainer
    {
        ...
        public virtual void ReadXml(System.Xml.XmlReader reader)
        {
            ...
            if (reader.MoveToAttribute("ContentSerializer"))
            {
                var value = reader.Value;
                if (value.IndexOf(',') > 0)
                {
                    var arr = value.Split(',');
                    var serializer = Assembly.Load(arr[1]).CreateInstance(arr[0]) as IXmlSerializer;
        ...
        public virtual void WriteXml(System.Xml.XmlWriter writer)
        {
            ...
            if (Content is IXmlSerializeContract)
            {
                var contract = (IXmlSerializeContract)Content;
                if (contract.CanSerialize)
                {
                    var serializer = contract.CreateSerializer();
                    var type = serializer.GetType();
                    writer.WriteAttributeString("ContentSerializer", type.FullName + "," + type.Assembly.GetName().Name);

...
namespace Css.Wpf.UI.Controls.Dashboards
{
    ...
    public class DragDockPanelHost : ItemsControl
    {
       ...
       // private MinimizedPositions minimizedPosition = MinimizedPositions.Right;
        ...
        public MinimizedPositions MinimizedPosition
        {
            get { return (MinimizedPositions)GetValue(MinimizedPositionProperty); }
            set { SetValue(MinimizedPositionProperty, value); }
        }

        // Using a DependencyProperty as the backing store for MinimizedPosition.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty MinimizedPositionProperty =
            DependencyProperty.Register("MinimizedPosition", typeof(MinimizedPositions), typeof(DragDockPanelHost), new UIPropertyMetadata(MinimizedPositions.Right,new PropertyChangedCallback(MinimizedPositionChangeCallBack)));
        ...
        protected virtual void PreparePanel(DragDockPanel panel)
        {
            ...
            panel.Closed += 
                new EventHandler(DragDockPanel_Closed);
        ...
        void DragDockPanel_Closed(object sender, EventArgs e)
        {
            var list = Items.SourceCollection as IList;
            if (list != null)
                list.Remove(sender);
        }
        ...
        {
            DragDockPanel ddg = (DragDockPanel)sender;
            if (ddg.PanelState != PanelState.Restored) return;          
        ...
        private static void MinimizedPositionChangeCallBack(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            var host = (DragDockPanelHost)d;
            host.AnimatePanelSizes();
            host.AnimatePanelLayout();
        }
        #endregion
    }
}